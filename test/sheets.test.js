...
test('buildHotkeysRowsFromSteps flattens steps into [stepName, hintLabel, hotkey], text and audio hints', () => {
  ...
  const steps = [
    ...
    {
      id: 'step_4', name: 'No usable text', order: 4,
      hints: [
        { id: 'h4', type: 'text', text: '', key: 'F5' },
        { id: 'h5', type: 'audio', key: 'F6' }, // no label, no mediaRef
      ],
    },
  ];
...
test('syncHotkeysTab sources its rows from config.hintGroups, not steps', async () => {
  const es = createEventStore({ path: ':memory:' });
  const gs = createGameStore(es.db);
  const fg = fakeGoogle();
  const cfg = { current: () => ({
